...
test.describe("reservation changes and cancellation", () => {
  test("an organizer requests a change, an operator applies it, then the organizer cancels", async ({
    page,
    browser,
    server,
  }) => {
    const password = "a solid change password";
    ...
    // Owner drafts, submits, and the operator approves a reservation.
    ...
    // The owner requests a capacity change on the approved reservation.
    await page.goto(reservationUrl);
    await expect(page.getByText("Approved", { exact: true })).toBeVisible();
    await page.getByLabel("Requested seats").fill("45");
    await page.getByRole("button", { name: "Submit change request" }).click();
    await expect(page.getByText(/Pending change request/i)).toBeVisible();

    // The operator reviews the change with its capacity impact and approves.
    await operatorPage.goto(`${server.serverUrl}/operator/changes?lang=en`);
    await operatorPage.getByRole("link", { name: "Review" }).first().click();
    await expect(operatorPage.getByText("Capacity impact")).toBeVisible();
    await operatorPage.getByRole("button", { name: "Approve" }).click();
    await expect(
      operatorPage.getByText("Applied", { exact: true }),
    ).toBeVisible();

    // The owner sees the applied change in the history, then cancels the event.
    await page.goto(reservationUrl);
    ...
    // A visitor no longer discovers the cancelled event.
    const visitorContext = await browser.newContext();
    const visitorPage = await visitorContext.newPage();
    await visitorPage.goto(`${server.serverUrl}/?lang=en`);
    await expect(visitorPage.getByText("Launch Party")).toHaveCount(0);
