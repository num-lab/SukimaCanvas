...
- [x] 回归测试至少覆盖一个三点转折：画板与导出投影都包含连续的 `C` 曲线，而规范存储仍为 `M/l`；PNG 像素断言能够区分平滑曲线与原始折线。
- [x] 运行 `npm test`、`npm run typecheck`，并按 Image Export 热路径约定记录 `npm run bench -- export` 的改动前后结果。
...
- 2026-09-09（tests）：新增 Published Canvas 平滑路径与 viewport、PNG 曲线像素、scale/shear 描边边界、legacy preview/export/download 与规范 baseline 分离，以及浏览器绘制／刷新／preview 一致性测试。同一断言在 `de67429` 快照分别捕获了折线路径、白色目标像素、变换描边边界低估和 preview 未平滑，当前分支目标测试全部通过。
- 完整验收：`npm run typecheck` 通过；`npm test` 中 Node 719 项为 717 通过、2 项因未配置 `WBO_TEST_POSTGRES_URL` 跳过，Playwright 86 项全部通过，Biome lint 通过。
- `npm run bench -- export` 默认 512-item 三样本：修复前 `de67429` 平均 2648.6 ms（2240.1 / 2257.3 / 3448.5 ms，7.5 MiB transient），当前平均 2221.9 ms（2190.1 / 2226.8 / 2248.9 ms，4.3 MiB transient）。输出因采用平滑曲线真实边界由 7211×3096 调整为 7203×3083；该样本未显示性能回退。
